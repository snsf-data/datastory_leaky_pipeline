...
# characters etc.)
datastory_name_de <- "je-hoeher-die-karrierestufe-desto-weniger-frauen"
datastory_name_en <- "proportion-of-women-decreasing-each-step-career-ladder"
datastory_name_fr <- "plus-la-hierarchie-academique-est-elevee-plus-les-femmes-sont-rares"

# English title and lead of the story (Mandatory, even if no EN version)
title_en <- "Gender monitoring (part 2): the proportion of women decreases with each step up the career ladder" 
lead_en <- "In Switzerland, more women than men study at universities. Yet women professors are in the minority. This imbalance is also reflected in the SNSF grant applications." 
# German title and lead of the story (Mandatory, even if no DE version)
title_de <- "Gender Monitoring (Teil 2): je höher die Karrierestufe, desto weniger Frauen" 
lead_de <- "An den universitären Hochschulen in der Schweiz studieren mehr Frauen als Männer. Professorinnen hingegen sind eine Minderheit. Dies zeigt sich ebenfalls bei den Gesuchen, die der SNF erhält." 
# French title and lead of the story (Mandatory, even if no FR version)
title_fr <- "Gender monitoring (2) : plus la hiérarchie académique est élevée, plus les femmes sont rares"
lead_fr <- "Bien qu'il y a plus de femmes que d'hommes qui étudient dans les universités suisses, les femmes professeurs sont en minorité. Ce déséquilibre se manifeste également dans les requêtes du FNS." 
# Contact persons, always (first name + last name)
contact_person <- c("Simona Isler", "Oliver John")
...
publication_date <- "2022-02-11 01:00:00"
